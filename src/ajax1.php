...
if($row['vencimiento']=="0000-00-00"){
    ...
    $fecha_vencimiento= date("d/m/Y", strtotime($row['vencimiento']));
       ...
       $data_tipo_value="";
       while ($row123 = mysqli_fetch_assoc($query_tipo)) {

           $data_tipo_value = $row123['tipo'];
       }
        ...
        $data['label'] = $row['codigo'] . ' - ' .$row['descripcion'].' - ' .$data_tipo_value.' - ' .$data_laboratorio_value. ' - ' .$fecha_vencimiento. ' - ' .$row['estante'];
    ...
    $id = $_SESSION['idUser'];
    $datos = array();
    $detalle = mysqli_query($conexion, "SELECT d.*, p.codproducto, p.descripcion,p.vencimiento,p.costo FROM detalle_temp d INNER JOIN producto p ON d.id_producto = p.codproducto WHERE d.id_usuario = $id");
...
if (isset($_POST['regDetalle'])) {
    ...
    $hoy = date('Y-m-d');
    $producto = mysqli_query($conexion, "SELECT *  FROM producto WHERE  estado = 0 AND codigo LIKE '%" . $nombre . "%' OR descripcion LIKE '%" . $nombre . "%' ORDER BY producto.codigo DESC LIMIT 5");
