...
header("Content-Disposition: attachment; filename=venta_dia_".date("d-m-Y").".xls");
...
$query = mysqli_query($conexion, "SELECT v.*, c.idcliente, c.nombre FROM ventas v INNER JOIN cliente c ON v.id_cliente = c.idcliente WHERE v.estado_venta = 0 AND v.fecha > CURDATE()");
